...
export default function Discovery() {
  ...
  async function like(row: DiscoverRow) {
    ...
    // Check whether a mutual match now exists.
  ...
  return (
    <AppShell>
      <div className="pt-8">
        <header className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`rounded-full border p-2.5 ${showFilters || onlineOnly || recentOnly || intent ? 'border-signal text-signal' : 'border-ink-line text-warm-mute'}`}
              data-testid="filter-toggle"
            >
              <SlidersHorizontal size={18} />
            </button>
            <button onClick={load} className="rounded-full border border-ink-line p-2.5 text-warm-mute" data-testid="discover-refresh">
              <RefreshCw size={18} />
            </button>
          </div>
        </header>

        <h1 className="display mt-6 text-4xl font-bold">Who&rsquo;s around</h1>
        <p className="mt-1 text-warm-mute">Active adults in Jakarta, right now.</p>

        <ConfigBanner />

        {suspended && (
          <div className="mt-5 rounded-2xl border border-signal/40 bg-signal/10 p-4 text-sm text-warm-white" data-testid="discover-suspended">
            Your account is {profile?.account_status}. Discovery is paused. See your profile for details.
          </div>
        )}
        ...
        {showFilters && (
          <div className="mt-5 card p-4" data-testid="filter-panel">
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setOnlineOnly((v) => !v)} className={`chip ${onlineOnly ? 'chip-on' : ''}`} data-testid="filter-online">
                Online now
              </button>
              <button onClick={() => setRecentOnly((v) => !v)} className={`chip ${recentOnly ? 'chip-on' : ''}`} data-testid="filter-recent">
                Recently active
              </button>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              {INTENTS.map((i) => (
                <button
                  key={i}
                  onClick={() => setIntent((cur) => (cur === i ? null : i))}
                  className={`chip ${intent === i ? 'chip-on' : ''}`}
                  data-testid={`filter-intent-${i}`}
                >
                  {i}
                </button>
              ))}
        ...
        {connectMsg && (
          <div
            className="mt-5 flex items-center gap-2 rounded-2xl border border-signal bg-signal/15 p-4 text-sm text-warm-white"
            data-testid="connect-toast"
            onClick={() => setConnectMsg(null)}
          >
            <Zap size={18} className="text-signal" /> {connectMsg}
          </div>
        )}

        <div className="mt-6 flex flex-col gap-6">
          {loading ? (
            <Spinner label="Finding people nearby…" />
          ) : rows.length === 0 ? (
            <div className="flex flex-col items-center py-14 text-center" data-testid="discover-empty">
              ...
              <p className="mt-2 max-w-xs text-warm-mute">
                No one matches yet. Try clearing filters, or check back soon — people come online
                throughout the day.
              </p>
            </div>
          ) : (
            rows.map((row) => (
              <ProfileCard
                ...
                onLike={like}
                onReport={(r) => setReport({ reportedUserId: r.id, context: 'profile' })}
                onBlock={block}
              />
            ))
          )}
