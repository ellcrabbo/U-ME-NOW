  ...
  return (
    <AuthLayout title="Welcome back" subtitle="Sign in to see who's around.">
      ...
      <form onSubmit={submit} className="flex flex-col gap-3">
        ...
        <button className="btn-signal mt-2" disabled={busy} data-testid="signin-submit">
          {busy ? 'Signing in…' : 'Sign in'}
        </button>
      </form>
      <div className="mt-4 flex items-center justify-between text-sm">
        <Link to="/auth/forgot" className="text-warm-mute" data-testid="signin-forgot">
          Forgot password?
        </Link>
        <Link to="/auth/sign-up" className="font-semibold text-signal" data-testid="signin-to-signup">
          Create account
        </Link>
      </div>
    </AuthLayout>
    ...
    return (
      <AuthLayout title="Check your email" subtitle={`We sent a confirmation link to ${email}.`}>
        <div className="card p-5 text-sm text-warm-mute" data-testid="signup-confirm-notice">
          ...
          <Link to="/auth/sign-in" className="mt-3 block text-center text-warm-mute underline">
            Back to sign in
          </Link>
        </div>
      </AuthLayout>
  ...
  return (
    <AuthLayout title="Join U, ME, NOW" subtitle="Adults 18+ in Jakarta.">
      ...
      <form onSubmit={submit} className="flex flex-col gap-3">
        ...
        <button className="btn-signal mt-2" disabled={busy} data-testid="signup-submit">
          {busy ? 'Creating…' : 'Create account'}
        </button>
      </form>
      <p className="mt-4 text-center text-sm text-warm-mute">
        Already have an account?{' '}
        <Link to="/auth/sign-in" className="font-semibold text-signal">
          Sign in
        </Link>
      </p>
    </AuthLayout>
