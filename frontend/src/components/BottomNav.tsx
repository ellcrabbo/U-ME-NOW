...
import { Compass, MessageCircle, UserRound, Shield } from 'lucide-react'
...
export function BottomNav() {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-40 mx-auto flex max-w-md items-stretch border-t border-ink-line bg-ink/95 backdrop-blur-md"
    ...
    >
      {items.map(({ to, label, icon: Icon, testid }) => (
        <NavLink
          ...
          data-testid={testid}
          className={({ isActive }) =>
            `${base} ${isActive ? 'text-signal' : 'text-warm-faint hover:text-warm-mute'}`
          }
        >
